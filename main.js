...
async function downloadSessionData() {
  ...
  try {
    if (_0xccedb8.SESSION_ID.startsWith('chatwise')) {
      // Handle Base64 encoded session
      const base64Data = _0xccedb8.SESSION_ID.split("Chatwise")[1];
      const decoded = Buffer.from(base64Data, 'base64').toString('utf-8');
      await _0x5687e2.promises.writeFile(credsPath, decoded);
      return true;
    } 
    else if (_0xccedb8.SESSION_ID.startsWith('chatwise')) {
      // Handle Pastebin session
      const pasteId = _0xccedb8.SESSION_ID.split("chatwise")[1];
...
async function start() {
  try {
    ...
    console.log("Chatwise is running on v" + _0x2f6d2f.join('.') + ", isLatest: " + _0x23f0a4);
    const _0x76bf4 = makeWASocket({
      ...
      'getMessage': async _0x53ca5a => {
        ...
        return {
          'conversation': "BEST WHATSAPP BOT MADE BY TOXIC"
        };
      }
    });
...
async function init() {
  if (_0x5687e2.existsSync(credsPath)) {
    ...
    if (_0x17d9d4) {
      console.log("chatwise IS RUNNING...⏳");
      await start();
    } else {
...
app.get('/', (_0x1ecf21, _0x282bcc) => {
  _0x282bcc.send("CHATWISE IS CONNECTED SUCCESSFULLY ✅");
});
app.listen(PORT, () => {
  console.log("chatwise daily users " + PORT);
});
